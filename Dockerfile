...
ENV MCFORGE_VERSION 11.15.1.1808
ENV MCFORGE_INSTALLER_JAR forge-installer.jar

ENV JAVA_XMX 1G
ENV JAVA_XMS 512M

ENV MINECRAFT_OPTS "-server -Xmx$JAVA_XMX -Xms$JAVA_XMS -XX:+AggressiveOpts -XX:+UseCompressedOops"
...
ENV SERVER_PORT 37210
ENV LEVEL_TYPE default
ENV ENABLE_RCON true
ENV RCON_PASSWORD 7654321
ENV RCON_PORT 25525
...
RUN chmod +x /usr/local/bin/minecraft; sync; /usr/local/bin/minecraft install
